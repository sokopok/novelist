#ifndef LIBAI_IMAGESRESPONSE_H
#define LIBAI_IMAGESRESPONSE_H

#include "imagesrequest.h"
#include "response.h"
...
class ImagesResponse : public ai::Response
{
    Q_OBJECT
    QML_NAMED_ELEMENT(ImagesResponse)
    QML_UNCREATABLE("ImagesResponse is created internally")

public:
    ImagesResponse(const ImagesRequest& request, QNetworkReply* reply, Client* client);

    [[nodiscard]] Client* client() const;
    [[nodiscard]] const ImagesRequest& request() const { return mRequest; }

    [[nodiscard]] ImagesRequest::Background background() const { return mBackground; }
    [[nodiscard]] QString backgroundAsString() const
    {
        return mBackground != ImagesRequest::Background_Custom
                   ? ImagesRequest::toString(mBackground)
    ...
    bool setBackground(const QString& background)
    {
        if (backgroundAsString() == background)
            return false;
        if (const auto b = ImagesRequest::toBackground(background);
            b != ImagesRequest::Background_Custom)
            return setBackground(b);
        extra().insert(QStringLiteral("background"), background);
        return setBackground(ImagesRequest::Background_Custom);
    }
    bool setBackground(ImagesRequest::Background background)
    {
        if (mBackground == background)
            return false;
        mBackground = background;
        if (mBackground != ImagesRequest::Background_Custom)
            extra().remove(QStringLiteral("background"));
        return true;
    }
    bool resetBackground() { return setBackground(ImagesRequest::Background_Auto); }
    ...
    [[nodiscard]] ImagesRequest::OutputFormat outputFormat() const { return mOutputFormat; }
    [[nodiscard]] QString outputFormatAsString() const
    {
        return mOutputFormat != ImagesRequest::OutputFormat_Custom
                   ? ImagesRequest::toString(mOutputFormat)
    ...
    bool setOutputFormat(const QString& outputFormat)
    {
        if (outputFormatAsString() == outputFormat)
            return false;
        if (const auto f = ImagesRequest::toOutputFormat(outputFormat);
            f != ImagesRequest::OutputFormat_Custom)
            return setOutputFormat(f);
        extra().insert(QStringLiteral("output_format"), outputFormat);
        return setOutputFormat(ImagesRequest::OutputFormat_Custom);
    }
    bool setOutputFormat(ImagesRequest::OutputFormat outputFormat)
    {
        if (mOutputFormat == outputFormat)
            return false;
        mOutputFormat = outputFormat;
        if (mOutputFormat != ImagesRequest::OutputFormat_Custom)
            extra().remove(QStringLiteral("output_format"));
        return true;
    }
    bool resetOutputFormat() { return setOutputFormat(ImagesRequest::OutputFormat_Png); }

    [[nodiscard]] ImagesRequest::Quality quality() const { return mQuality; }
    [[nodiscard]] QString qualityAsString() const
    {
        return mQuality != ImagesRequest::Quality_Custom
                   ? ImagesRequest::toString(mQuality)
    ...
    bool setQuality(const QString& quality)
    {
        if (qualityAsString() == quality)
            return false;
        if (const auto q = ImagesRequest::toQuality(quality); q != ImagesRequest::Quality_Custom)
            return setQuality(q);
        extra().insert(QStringLiteral("quality"), quality);
        return setQuality(ImagesRequest::Quality_Custom);
    }
    bool setQuality(ImagesRequest::Quality quality)
    {
        if (mQuality == quality)
            return false;
        mQuality = quality;
        if (mQuality != ImagesRequest::Quality_Custom)
            extra().remove(QStringLiteral("quality"));
        return true;
    }
    bool resetQuality() { return setQuality(ImagesRequest::Quality_Auto); }

    [[nodiscard]] ImagesRequest::Size size() const { return mSize; }
    [[nodiscard]] QString sizeAsString() const
    {
        return mSize != ImagesRequest::Size_Custom
                   ? ImagesRequest::toString(mSize)
    ...
    bool setSize(const QString& size)
    {
        if (sizeAsString() == size)
            return false;
        if (const auto s = ImagesRequest::toSize(size); s != ImagesRequest::Size_Custom)
            return setSize(s);
        extra().insert(QStringLiteral("size"), size);
        return setSize(ImagesRequest::Size_Custom);
    }
    bool setSize(ImagesRequest::Size size)
    {
        if (mSize == size)
            return false;
        mSize = size;
        if (mSize != ImagesRequest::Size_Custom)
            extra().remove(QStringLiteral("size"));
        return true;
    }
    bool resetSize() { return setSize(ImagesRequest::Size_Auto); }
...
protected:
    ...
    ImagesRequest mRequest;
    ImagesRequest::Background mBackground = ImagesRequest::Background_Auto;
    int mCreated = 0;
    ImageResponseData mData;
    ImagesRequest::OutputFormat mOutputFormat = ImagesRequest::OutputFormat_Png;
    ImagesRequest::Quality mQuality = ImagesRequest::Quality_Auto;
    ImagesRequest::Size mSize = ImagesRequest::Size_Auto;
