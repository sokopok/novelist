...
bool ResponsesRequest::readJson(const QJsonObject &json, QStringList *errors)
{
    bool ok = Request::readJson(json);
    ...
    if (mExtra.contains(QStringLiteral("input"))) {
        if (const auto v = mExtra.value(QStringLiteral("input")); v.isArray()) {
            ...
            if (errors)
                errors->append("input is not an array");
            ok = false;
        }
    }
...
bool ResponsesRequest::writeJson(QJsonObject &json, bool full) const
{
    if (!Request::writeJson(json, full))
        return false;
    ...
    if (full || (!mReasoning.isEmpty() && isReasoningModel()))
    ...
    if (full || !mTools.isEmpty()) {
        QJsonArray tools;
        for (const auto &tool : mTools)
            tools.append(tool.toJson());
        json[QStringLiteral("tools")] = tools;
    }
...
const QMap<int, QString> ResponsesRequest::ModelKV{{Model_Gpt5, QStringLiteral("gpt-5")},
...
const QMap<int, QString> ResponsesRequest::ServiceTierKV{{ServiceTier_Auto, QStringLiteral("auto")},
                                                         {ServiceTier_Default,
...
const QMap<int, QString> ResponsesRequest::TruncationKV{{Truncation_Auto, QStringLiteral("auto")},
                                                        {Truncation_Disabled,
