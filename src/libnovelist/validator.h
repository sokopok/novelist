#ifndef NOVELIST_VALIDATOR_H
#define NOVELIST_VALIDATOR_H
...
public:
    ...
    virtual bool fixup(QVariant &input, QStringList *messages = nullptr) const = 0;
    virtual bool validate(QVariant &input, QStringList *messages = nullptr) const = 0;
...
public:
    ...
    bool fixup(QVariant &input, QStringList *messages = nullptr) const override
    {
        auto string = input.toString();
    ...
    bool validate(QVariant &input, QStringList *messages = nullptr) const override
    {
        auto string = input.toString().trimmed();
...
public:
    ...
    bool fixup(QVariant &input, QStringList *messages = nullptr) const override
    {
        if (required() && !input.isValid()) {
            if (messages)
                messages->append(QStringLiteral("Required."));
            input = false;
        }

        if (input.typeId() != qMetaTypeId<bool>()) {
            if (messages)
...
public:
    ...
    bool fixup(QVariant &input, QStringList *messages = nullptr) const override
    {
        QDateTime dateTime = input.toDateTime();
