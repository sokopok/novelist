...
public:
    ...
    {
        return mModel != Request::Model_Custom ? Request::toString(mModel)
    ...
    bool resetModel() { return setModel(Request::Model_DallE2); }
    ...
    bool setModeration(const QString& moderation)
    {
        if (moderationAsString() == moderation)
            return false;
        if (Request::Moderation m = Request::toModeration(moderation);
            m != Request::Moderation_Custom)
            return setModeration(m);
        extra().insert(QStringLiteral("moderation"), moderation);
        return setModeration(Request::Moderation_Custom);
    }
    ...
    bool setN(int n)
    {
        n = std::max(1, std::min(model() == Request::Model_DallE3 ? 1 : 10, n));
        if (mN == n)
            return false;
        mN = n;
        return true;
    ...
    bool setResponseFormat(const QString& responseFormat)
    {
        if (responseFormatAsString() == responseFormat)
            return false;
        if (const auto f = Request::toResponseFormat(responseFormat);
            f != Request::ResponseFormat_Custom)
            return setResponseFormat(f);
        extra().insert(QStringLiteral("response_format"), responseFormat);
        return setResponseFormat(Request::ResponseFormat_Custom);
    }
    ...
    bool setStyle(const QString& style)
    {
        if (styleAsString() == style)
            return false;
        if (const auto s = Request::toStyle(style); s != Request::Style_Custom)
            return setStyle(s);
        extra().insert(QStringLiteral("style"), style);
        return setStyle(Request::Style_Custom);
    }
